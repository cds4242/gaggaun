...
### 헤더 CTA 변경 — 새가족 등록 → 오시는 길

- `web/src/components/site-header.tsx` — 우상단 황금색 CTA 버튼 라벨 "새가족 등록" → "**오시는 길**", 링크 `/new-member` → `/about/location`. 데스크톱 nav + 모바일 메뉴 두 위치 모두.
- 이유: 첫 방문자에게 더 보편적으로 유용한 동선이 위치/교통 안내라는 운영 판단. 새가족 등록 진입은 헤더 인사말 카드·메인 빠른이동 카드·푸터 바로가기·상단 util-bar에 그대로 유지되므로 접근성은 손실 없음.
